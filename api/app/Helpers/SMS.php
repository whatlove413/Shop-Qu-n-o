<?php
/*
Vậy hiện giờ các đầu số mình sử dụng sẽ như sau:
Vina, Viettel, VNM, Gtel: alias là SharingEco
Mobi: 0901766199
- http://thuthuat123.com/danh-sach-dau-so-dien-thoai-cac-nha-mang-o-viet-nam-moi-nhat/ (đầu số điện thoại di động)
*/
...
class Sms
{
    ...
    public function __construct($options = [])
    {
        ...
        $this->brand2 = $options["ALIAS"] ? : env('ALIAS2');
        $this->brand3 = $options["ALIAS"] ? : env('ALIAS3');
        $this->brand4 = $options["ALIAS"] ? : env('ALIAS4');
    ...
    {
        $VIETNAMMOBILE = ['092','0188','058','0186','056','0199','059'];
        $VINAPHONE = ['088','091','094','0123','083','0124','084','0125','085','0127','081','0129','082'];
        $MOBILEPHONE = ['0120','070','0121','079','0122','077','0126','076','0128','078','090','093','089'];
        $VIETTEL = ['0162','032','0163','033','0164','034','0165','035','0166','036','0167','037','0168','038','0169','039','086','096','097','098'];

        if(preg_match('!!u', $phoneNumbers))
        {
            $phoneNumbers = str_replace("?", "", utf8_decode($phoneNumbers));
        }
        /*
        if(in_array(substr($phoneNumbers, 0, 4), $VIETNAMMOBILE) || in_array(substr($phoneNumbers, 0, 3), $VIETNAMMOBILE))
        {
        ...
        //Mobi: 0901766199
        if(in_array(substr($phoneNumbers, 0, 4), $MOBILEPHONE) || in_array(substr($phoneNumbers, 0, 3), $MOBILEPHONE))
        {
            $data_string = '<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
            <Body>
            <BulkMessageBlockReciver xmlns="http://tempuri.org/">
