    ...
    function uploadManyFile( $fileMany, $urlIn )
    {
        ...
        if( $fileMany )
        {
            ...
            foreach( $fileMany as $fileIn )
            {
                ...
                $img->save($path_Pro); //save created image (will override old image)
                //resize hình thành cỡ lớn lưu vào file large
                $image_large = Image::make($path_Pro)->resize(270, 172);
    ...
    function uploadManyFileEnterprise( $fileMany, $urlIn, $type )
    {
        ...
        if( $fileMany )
        {
            ...
            foreach( $fileMany as $fileIn )
            {
                ...
                if($width >336 && $type == 1){
                    //resize hình thành cỡ lớn lưu vào file large
                    $image_large = Image::make($path_Pro)->resize(336, 300);
                    $image_large->save($url."/../large/".$name);
                }
                //resize thành ảnh large recruitment $type = 2
                elseif($width > 266 && $type == 2)
                {
                    //resize hình thành cỡ lớn lưu vào file large
                    $image_large = Image::make($path_Pro)->resize(266, 172);
                    $image_large->save($url."/../large/".$name);
                }
                else {
